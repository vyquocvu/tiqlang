...
static const char TIQ_RUNTIME_PRELUDE_AUX9[] =
    "static void *tiq_container_alloc(uint64_t a, size_t n, size_t al) {\n"
    "    if (a) {\n"
    "        void *p = (void *)(uintptr_t)tiq_allocator_alloc(a, (uint64_t)n, (uint64_t)(al ? al : _Alignof(max_align_t)));\n"
    ...
    "static const char *tiq_container_dup_str(uint64_t a, const char *s) {\n"
    "    if (!s) s = \"\";\n"
    "    size_t n = strlen(s);\n"
    "    char *d = (char *)tiq_container_alloc(a, n + 1, 1);\n"
    "    memcpy(d, s, n + 1);\n"
    "    return d;\n"
    "}\n\n"

    "static void tiq_container_dealloc(uint64_t a, void *p, size_t n, size_t al) {\n"
    "    if (a && p) {\n"
    "        tiq_allocator_dealloc(a, (uint64_t)(uintptr_t)p, (uint64_t)n, (uint64_t)(al ? al : _Alignof(max_align_t)));\n"
    "    }\n"
    "}\n\n"

    "typedef struct { unsigned char *data; int64_t len; int64_t cap; int64_t elem_size; uint64_t allocator; } TiqVec;\n\n"

    "static TiqVec *tiq_vec_with_allocator(uint64_t allocator) {\n"
    "    TiqVec *v = (TiqVec *)tiq_container_alloc(allocator, sizeof(TiqVec), _Alignof(max_align_t));\n"
    "    v->data = NULL;\n"
    "    v->len = 0;\n"
    "    v->cap = 0;\n"
    "    v->elem_size = 0;\n"
    ...
    "    return tiq_vec_with_allocator(0);\n"
    "}\n\n"
    ...
    "    unsigned char *nd = (unsigned char *)tiq_container_alloc(v->allocator, (size_t)(nc * v->elem_size), _Alignof(max_align_t));\n"
    "    if (v->len > 0) memcpy(nd, v->data, (size_t)(v->len * v->elem_size));\n"
    "    if (v->allocator && v->data && v->cap > 0) {\n"
    "        tiq_container_dealloc(v->allocator, v->data, (size_t)(v->cap * v->elem_size), _Alignof(max_align_t));\n"
    "    }\n"
    "    v->data = nd;\n"
    "    v->cap = nc;\n"
    "}\n\n"
    ...
    "    const char *d = tiq_container_dup_str(v->allocator, s);\n"
...
static const char TIQ_RUNTIME_PRELUDE_AUX10[] =
    "typedef struct { char *data; int64_t len; int64_t cap; uint64_t allocator; } TiqStrBuf;\n\n"

    "static TiqStrBuf *tiq_str_buf_with_allocator(uint64_t allocator) {\n"
    "    TiqStrBuf *b = (TiqStrBuf *)tiq_container_alloc(allocator, sizeof(TiqStrBuf), _Alignof(max_align_t));\n"
    "    b->cap = 16;\n"
    "    b->allocator = allocator;\n"
    "    b->data = (char *)tiq_container_alloc(allocator, (size_t)b->cap, 1);\n"
    "    b->data[0] = '\\0';\n"
    "    b->len = 0;\n"
    "    return b;\n"
    "}\n\n"
    ...
    "    return tiq_str_buf_with_allocator(0);\n"
    "}\n\n"
    ...
    "        int64_t nc = b->cap;\n"
    "        while (nc < need) nc *= 2;\n"
    "        char *nd = (char *)tiq_container_alloc(b->allocator, (size_t)nc, 1);\n"
    "        memcpy(nd, b->data, (size_t)b->len + 1);\n"
    "        if (b->allocator && b->data && b->cap > 0) {\n"
    "            tiq_container_dealloc(b->allocator, b->data, (size_t)b->cap, 1);\n"
    "        }\n"
    "        b->data = nd;\n"
    "        b->cap = nc;\n"
    "    }\n"
...
static const char TIQ_RUNTIME_PRELUDE_AUX11[] =
    "typedef struct { const char **keys; int64_t *vals; int64_t len; int64_t cap; int64_t *buckets; int64_t nbuckets; uint64_t allocator; } TiqMap;\n\n"
    ...
    "static TiqMap *tiq_map_with_allocator(uint64_t allocator) {\n"
    "    TiqMap *m = (TiqMap *)tiq_container_alloc(allocator, sizeof(TiqMap), _Alignof(max_align_t));\n"
    "    m->keys = NULL;\n"
    "    m->vals = NULL;\n"
    "    m->len = 0;\n"
    "    m->cap = 0;\n"
    ...
    "    m->buckets = (int64_t *)tiq_container_alloc(allocator, (size_t)m->nbuckets * sizeof(int64_t), _Alignof(max_align_t));\n"
    "    for (int64_t i = 0; i < m->nbuckets; i++) m->buckets[i] = -1;\n"
    "    return m;\n"
    "}\n\n"

    "static TiqMap *tiq_map_new(void) {\n"
    "    return tiq_map_with_allocator(0);\n"
    "}\n\n"

    "static void tiq_map_rehash(TiqMap *m) {\n"
    "    int64_t old_nbuckets = m->nbuckets;\n"
    "    int64_t *old_buckets = m->buckets;\n"
    "    m->nbuckets *= 2;\n"
    "    m->buckets = (int64_t *)tiq_container_alloc(m->allocator, (size_t)m->nbuckets * sizeof(int64_t), _Alignof(max_align_t));\n"
    "    for (int64_t i = 0; i < m->nbuckets; i++) m->buckets[i] = -1;\n"
    "    for (int64_t e = 0; e < m->len; e++) tiq_map_place(m, e);\n"
    "    if (m->allocator && old_buckets) {\n"
    "        tiq_container_dealloc(m->allocator, old_buckets, (size_t)old_nbuckets * sizeof(int64_t), _Alignof(max_align_t));\n"
    "    }\n"
    "}\n\n"
    ...
    "        const char **nk = (const char **)tiq_container_alloc(m->allocator, (size_t)nc * sizeof(const char *), _Alignof(max_align_t));\n"
    "        int64_t *nv = (int64_t *)tiq_container_alloc(m->allocator, (size_t)nc * sizeof(int64_t), _Alignof(max_align_t));\n"
    "        if (m->len > 0) {\n"
    "            memcpy(nk, m->keys, (size_t)m->len * sizeof(const char *));\n"
    "            memcpy(nv, m->vals, (size_t)m->len * sizeof(int64_t));\n"
    "        }\n"
    "        if (m->allocator && m->cap > 0) {\n"
    "            if (m->keys) tiq_container_dealloc(m->allocator, (void*)m->keys, (size_t)m->cap * sizeof(const char *), _Alignof(max_align_t));\n"
    "            if (m->vals) tiq_container_dealloc(m->allocator, (void*)m->vals, (size_t)m->cap * sizeof(int64_t), _Alignof(max_align_t));\n"
    "        }\n"
    "        m->keys = nk;\n"
    "        m->vals = nv;\n"
    "        m->cap = nc;\n"
    "    }\n"
    "    m->keys[m->len] = tiq_container_dup_str(m->allocator, kk);\n"
    "    m->vals[m->len] = v;\n"
...
static const char TIQ_RUNTIME_PRELUDE_AUX11B[] =
...
// M19.2: Thread-Safe Channel Runtime Primitives (LANGUAGE_SPEC §19.12)
static const char TIQ_RUNTIME_PRELUDE_AUX12[] =
    "typedef struct {\n"
    "    int64_t *buf;\n"
    "    int64_t cap;\n"
    "    int64_t head;\n"
    ...
    "static uint64_t tiq_chan_new(int64_t cap) {\n"
    "    if (cap <= 0) cap = 1;\n"
    "    TiqChan *ch = (TiqChan *)malloc(sizeof(TiqChan));\n"
    "    if (!ch) return 0;\n"
    ...
    "static int64_t tiq_chan_send(uint64_t handle, int64_t val) {\n"
    "    TiqChan *ch = (TiqChan *)(uintptr_t)handle;\n"
    "    if (!ch) return -1;\n"
    ...
    "static int64_t tiq_chan_recv(uint64_t handle) {\n"
    "    TiqChan *ch = (TiqChan *)(uintptr_t)handle;\n"
    "    if (!ch) return 0;\n"
    "    pthread_mutex_lock(&ch->mtx);\n"
    ...
    "static int64_t tiq_chan_try_recv(uint64_t handle) {\n"
    "    TiqChan *ch = (TiqChan *)(uintptr_t)handle;\n"
    "    if (!ch) return 0;\n"
    "    pthread_mutex_lock(&ch->mtx);\n"
    "    if (ch->count == 0) {\n"
    ...
    "static int64_t tiq_chan_close(uint64_t handle) {\n"
    "    TiqChan *ch = (TiqChan *)(uintptr_t)handle;\n"
    "    if (!ch) return -1;\n"
    "    pthread_mutex_lock(&ch->mtx);\n"
    "    ch->closed = 1;\n"
    ...
    "static int64_t tiq_chan_len(uint64_t handle) {\n"
    "    TiqChan *ch = (TiqChan *)(uintptr_t)handle;\n"
    "    if (!ch) return 0;\n"
    "    pthread_mutex_lock(&ch->mtx);\n"
    ...
    "static int64_t tiq_chan_cap(uint64_t handle) {\n"
    "    TiqChan *ch = (TiqChan *)(uintptr_t)handle;\n"
    "    return ch ? ch->cap : 0;\n"
    "}\n\n"

    "static int64_t tiq_chan_free(uint64_t handle) {\n"
    "    TiqChan *ch = (TiqChan *)(uintptr_t)handle;\n"
    "    if (!ch) return -1;\n"
