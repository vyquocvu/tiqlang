...
`[deps]` source values use a scheme prefix to identify the dependency type (M18.1):
- `path:<dir>` — local directory path (copied to `.tiq-deps/<name>/` by `tiq install`)
- `git:<url>` — git repository (cloned by `tiq install`)
- `git:<url>#<ref>` — git repository at a specific tag, branch, or commit
