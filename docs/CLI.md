...
`tiq emit-obj <file.s> -o <file.o>` (M17.3.1) assembles the QBE native assembly subset in-process and writes a deterministic relocatable object (Mach-O on Darwin arm64, ELF64 on Linux aarch64/x86_64), using the same assembler and object writer that `tiq build --backend qbe` uses internally. It is only available on those hosts: invoking it elsewhere prints a diagnostic and exits 2. Any instruction or directive outside the backend's assembly subset fails closed with a `<file>:<line>: error` diagnostic, and the output file is removed on failure.

`tiq link-qbe <obj>... -o <exe>` (M17.3.2) links relocatable objects into a runnable executable entirely in-process, without invoking `cc`/`ld`: a self-signed arm64 `MH_EXECUTE` on Darwin through dyld stubs against a fixed libSystem export set (Mach-O), or an ELF64 `ET_EXEC` with GOT/PLT and `DT_NEEDED` for libc.so.6 (Linux aarch64/x86_64, driven by the dynamic linker through `PT_INTERP`). Undefined symbols are resolved against the input objects first and then the platform export set; anything unresolved fails closed naming the symbol. Output is deterministic: identical inputs produce byte-identical executables (no timestamps; a UUID derived from the input content on Mach-O). It is only available on the Darwin arm64 and Linux aarch64/x86_64 hosts, and exits 2 elsewhere. Failures (missing input, non-object input, unresolved symbol) exit 1 and never leave a partial executable behind.
...
- `tiq router <port>` runs the sequential loopback routed-service dogfood artifact. Routes are `GET /health`, `GET /hello/<name>`, and `POST /echo`. Requests are capped at 64 KiB including headers and body; the service returns `400` for incomplete headers, `413` for oversized or incomplete declared bodies, `404` for unknown routes, and `405` for known routes with the wrong method. Every response declares `Content-Length` and closes the connection. Missing or invalid ports exit 2; an unbindable port exits 1. This is an application built by the bootstrap (`src/tiq/tools/router.tiq`), not a subcommand embedded in the C compiler.
