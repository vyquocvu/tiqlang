...
- M11 (queued): LSP `hover`/`definition`/`semanticTokens` answer with real front-end data since 2026-07-27 (plan 5.1): hover shows the declared symbol's inferred type, definition returns the declaration token's range, semantic tokens are delta-encoded real lexer token kinds. Responses are deterministic per stored `(uri, version)`; unknown uris/positions and unsupported methods fail closed with `null`. M11.1 (2026-07-29): `didOpen` publishes structured in-protocol diagnostics from the real front end (docs/CLI.md). Windows platform layer, wasm playground, and self-hosted compiler remain open.
