...
- M10 (active): M10.1 CLI argument builtins implemented 2026-07-29 — `cli_arg_count()` and `cli_arg(i)` (LANGUAGE_SPEC §18.1) backed by real `argc`/`argv`; emitted `main` is now `int main(int argc, char **argv)`; out-of-range index yields the empty string. `net_fetch` is still a hardcoded stub; no event loop, sockets, or HTTP code exists. `json_parse_int`/`json_encode_str` are minimal helpers.
