...
- [ ] **M21.3** Dogfooding: Build core infrastructure tools in Tiq (e.g. fast CLI tools, edge proxy). Progress 2026-08-03: first artifact shipped — `tiq proxy` (`src/tiq/tools/proxy.tiq`, ~185 lines), a loopback HTTP reverse proxy written entirely in Tiq on the std/net socket primitives: Content-Length-aware request forwarding (256 KiB cap), streamed responses, fail-closed 400/413/502 paths. Evidence: `tests/proxy_tool.sh` (30th suite, wired into `make test` and `tool-proxy`) — GET/POST passthrough against a Tiq upstream fixture, 502 on dead upstream, usage fail-closed, ASan/UBSan clean. Concurrency stays sequential by design until the M19 event loop lands. Continuous: more infrastructure tools follow.
