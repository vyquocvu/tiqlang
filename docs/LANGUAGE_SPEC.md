...
Bootstrap limits: temporaries outside the positions above are not destroyed, owners in calling functions are not destroyed by `proc_exit`, and strings transferred out of a function are not destroyed by the caller. These paths leak; they never double-free or dangle.
