#!/bin/sh
# M17.4.1: integrated ELF64 executable linker tests (riscv64 Linux host).
#
...
# On non-Linux-riscv64 hosts the suite is skipped.
set -eu

TIQ=./build/tiq
TMP_DIR="${TMPDIR:-/tmp}/tiq-rv64-link-$$"
mkdir -p "$TMP_DIR"
trap 'rm -rf "$TMP_DIR"' EXIT INT TERM

HOST="$(uname -s)-$(uname -m)"
if [ "$HOST" != "Linux-riscv64" ]; then
  echo "rv64_link: skipped (host is $HOST, integrated ELF linker is Linux-riscv64 only)"
  exit 0
fi

if [ ! -x "$TIQ" ] || [ ! -x ./build/qbe ] || [ ! -f ./build/runtime_qbe.o ]; then
  echo "rv64_link: missing build artifacts (tiq, qbe, runtime_qbe.o)" >&2
  exit 1
fi
...
echo "RISC-V ELF executable linker tests passed"