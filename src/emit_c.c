...
static void emit_expr(AstNode *node, EmitContext *ctx) {
    ...
    switch (node->kind) {
        ...
        case AST_CHAN:
            ...
            break;
        case AST_MATCH:
            // Pre-M13 S1: dedicated emit_match handles collision-free
            // names, recursive constructor pattern lowering, and
            // byte-equality for string patterns (LANGUAGE_SPEC §17.1).
            emit_match(node, ctx);
            break;
        case AST_BLOCK:
        ...
        // Pre-M13 S1: recursive pattern condition emission. Emits a C boolean
// expression that is true exactly when `pat` matches the value reachable
// through the textual accessor `path` (which has static type `scrut_t`).
// Empty string means "always true" — caller wraps with a no-op `if ()`.
// `path` is owned by the caller (always a string literal stack buffer).
static void emit_pat_cond(Pattern *pat, SemanticType *scrut_t, const char *path, EmitContext *ctx) {
    if (!pat) return;
    switch (pat->kind) {
        case PAT_WILDCARD:
            // Always matches; emit nothing so caller emits unconditional block.
            break;
        case PAT_LITERAL: {
            // str patterns use byte-equality (tiq_str_eq) — never C
            // pointer equality (LANGUAGE_SPEC §17.1). All other kinds
            // compare with `==`, which is sound for int/float/bool/enum
            // and works on the field-access path for Option/Result.
            SemanticType *lit_t = pat->as.literal.expr ?
                (SemanticType *)pat->as.literal.expr->semantic_type : NULL;
            bool str_lit = lit_t && lit_t->kind == TYPE_STR;
            // none literal matches Option/Result's emptiness
            bool none_lit = lit_t && lit_t->kind == TYPE_OPTION;
            bool none_result_lit = false;
            // Distinguish literal type: a none literal node has kind TYPE_OPTION;
            // the scrutinee tells us Option vs Result for the field name.
            if (lit_t && lit_t->kind == TYPE_UNKNOWN) {
                // PAT_NONE may type-infer as TYPE_UNKNOWN when the literal
                // node carries only the token kind; rely on scrut_t.
                TokenKind lt = pat->as.literal.expr->as.literal.type;
                if (lt == TOK_NONE) {
                    if (scrut_t && scrut_t->kind == TYPE_OPTION) {
                        none_lit = true;
                    } else if (scrut_t && scrut_t->kind == TYPE_RESULT) {
                        none_lit = true;
                        none_result_lit = true;
                    }
                }
                str_lit = lt == TOK_STRING;
            }
            if (none_lit) {
                fputs("!(", ctx->out);
                fputs(path, ctx->out);
                if (none_result_lit) fputs(".is_ok", ctx->out);
                else fputs(".has_value", ctx->out);
                fputs(")", ctx->out);
            } else if (str_lit) {
                fputs("tiq_str_eq(", ctx->out);
                fputs(path, ctx->out);
                fputs(", ", ctx->out);
        ...
        case PAT_BINDING:
            // Bare binding always matches — emit nothing.
            break;
        case PAT_CONSTRUCTOR: {
            ...
            Pattern *sub = (pat->as.constructor.arg_count >= 1)
                ? pat->as.constructor.args[0] : NULL;
            SemanticType *inner_t = NULL;
            const char *field_name = NULL;
            if (is_some) { inner_t = scrut_t ? scrut_t->inner_type : NULL; field_name = "value"; }
            else if (is_ok) { inner_t = scrut_t ? scrut_t->inner_type : NULL; field_name = "value"; }
            else if (is_err) { inner_t = scrut_t ? scrut_t->error_type : NULL; field_name = "error"; }
            fputs("(", ctx->out);
            if (is_err) fputs("!", ctx->out);
            fputs(path, ctx->out);
            if (is_some) fputs(".has_value", ctx->out);
            else if (is_ok) fputs(".is_ok", ctx->out);
            else if (is_err) fputs(".is_ok", ctx->out);
            bool emitted_inner = false;
            if (sub && (is_some || is_ok || is_err)) {
                char sub_path[256];
                snprintf(sub_path, sizeof sub_path, "%s.%s", path, field_name);
                bool has_sub = (sub->kind != PAT_WILDCARD &&
                                sub->kind != PAT_BINDING);
                if (has_sub) {
                    fputs(" && ", ctx->out);
                    emit_pat_cond(sub, inner_t, sub_path, ctx);
                    emitted_inner = true;
                }
            }
            (void)emitted_inner;
...
// Pre-M13 S1: recursive pattern binding emission. Emits one or more C
// statements that bind the values reachable through `path` to the pattern's
// binding names. Empty for non-binding sub-patterns.
static void emit_pat_binds(Pattern *pat, SemanticType *scrut_t, const char *path, EmitContext *ctx) {
    if (!pat) return;
    switch (pat->kind) {
        case PAT_WILDCARD:
        case PAT_LITERAL:
        case PAT_ENUM_VARIANT:
            break;
        case PAT_BINDING: {
            if (scrut_t) emit_semantic_type(scrut_t, ctx->out);
        ...
        case PAT_CONSTRUCTOR: {
            ...
            Pattern *sub = (pat->as.constructor.arg_count >= 1)
                ? pat->as.constructor.args[0] : NULL;
            if (is_some && sub) {
                snprintf(sub_path, sizeof sub_path, "%s.value", path);
                emit_pat_binds(sub, scrut_t ? scrut_t->inner_type : NULL,
                               sub_path, ctx);
            } else if (is_ok && sub) {
                snprintf(sub_path, sizeof sub_path, "%s.value", path);
                emit_pat_binds(sub, scrut_t ? scrut_t->inner_type : NULL,
                ...
                snprintf(sub_path, sizeof sub_path, "%s.error", path);
                emit_pat_binds(sub, scrut_t ? scrut_t->error_type : NULL,
                               sub_path, ctx);
            }
            break;
        }
...
// Emit a single C statement that contains the if/else chain assigning
// the match result. The scrutinee is referenced by name (`scrut_name`,
// of type `scrut_t`); each arm's condition and bindings are emitted
// against that path. The result is written to `result_name`.
static void emit_match_chain(AstNode *node, const char *scrut_name,
                             const char *result_name, SemanticType *scrut_t,
                             EmitContext *ctx) {
    bool first = true;
    for (int i = 0; i < node->as.match_expr.arm_count; i++) {
        MatchArm *arm = &node->as.match_expr.arms[i];
        Pattern *pat = arm->pat;
        bool wildcard_like = pat &&
            (pat->kind == PAT_WILDCARD || pat->kind == PAT_BINDING);
        if (wildcard_like) {
            fputs("{ ", ctx->out);
        } else {
            fputs("if (", ctx->out);
            emit_pat_cond(pat, scrut_t, scrut_name, ctx);
            fputs(") { ", ctx->out);
        }
        if (!first) {} // no-op for readability
        first = false;
        emit_pat_binds(pat, scrut_t, scrut_name, ctx);
        fputs(result_name, ctx->out);
        fputs(" = ", ctx->out);
        emit_expr(arm->body, ctx);
        fputs("; } ", ctx->out);
        if (!wildcard_like) fputs("else ", ctx->out);
    }
}
static void emit_match(AstNode *node, EmitContext *ctx) {
    ...
    fputs("; ", ctx->out);
    if (res_t) emit_semantic_type(res_t, ctx->out);
    else fputs("int64_t", ctx->out);
    fputs(" _r; ", ctx->out);
    emit_match_chain(node, "_t", "_r", scrut_t, ctx);
    fputs("_r; })", ctx->out);
}
...
// M12.6: struct definitions (before function declarations so types are
// visible). Shared by the program pass and the M16.3 header pass.
static void emit_struct_defs(EmitContext *ctx, AstNode **stmts, int count) {
...
// M16.3: header export filter — the FFI-safe set of the C ABI table
// (LANGUAGE_SPEC §7.1). Unknown types follow the emitted-C convention
// (int64_t) and are spelled identically by definition and prototype.
static bool header_safe_type(SemanticType *t) {
    if (!t) return true;
    if (t->kind == TYPE_STRUCT) return t->struct_name && t->struct_name[0];
    return t->kind == TYPE_UNKNOWN ||
           t->kind == TYPE_INT || t->kind == TYPE_I8 || t->kind == TYPE_I16 ||
           t->kind == TYPE_I32 || t->kind == TYPE_U8 || t->kind == TYPE_U16 ||
           t->kind == TYPE_U32 || t->kind == TYPE_U64 || t->kind == TYPE_F32 ||
           t->kind == TYPE_FLOAT || t->kind == TYPE_BOOL || t->kind == TYPE_STR;
}

// M16.3: a function is exported in the header when every parameter is
// annotated FFI-safe (or unannotated and still unknown, so both compilers
// spell int64_t), carries no borrow, and the return is FFI-safe.
// Stream-gen bodies and extern decls never export.
static bool function_exportable(AstNode *fn) {
    if (fn->kind != AST_FUNCTION) return false;
    if (fn->as.function.body && fn->as.function.body->kind == AST_STREAM_GEN)
        return false;
    for (int i = 0; i < fn->as.function.param_count; i++) {
        if (fn->as.function.param_ref_kinds && fn->as.function.param_ref_kinds[i] != 0)
            return false;
        bool annotated = fn->as.function.param_type_annots &&
                         fn->as.function.param_type_annots[i].kind == TOK_IDENT;
        SemanticType *pt = fn->as.function.param_types ?
            (SemanticType *)fn->as.function.param_types[i] : NULL;
        if (annotated) {
            if (!header_safe_type(pt)) return false;
        } else if (pt && pt->kind != TYPE_UNKNOWN) {
            // Inference moved an unannotated parameter: the self-hosted
            // emitter cannot see it, so keep the function internal.
            return false;
        }
    }
    return header_safe_type((SemanticType *)fn->semantic_type);
}

// M16.3: include-guard body — basename uppercased, non-alphanumerics
// mapped to '_'. Emitted twice per header (#ifndef / #define).
static void fputs_header_guard(FILE *out, const char *base, size_t blen) {
...
// M16.3: deterministic C header for a definitions-only library. The
// prototypes use the same emit_semantic_type path as the --lib function
// definitions, so declaration and definition always match.
static void emit_c_header(EmitContext *ctx, AstNode **stmts, int count, const char *root_path) {
    const char *base = root_path;
    for (const char *p = root_path; *p; p++)
        if (*p == '/') base = p + 1;
    size_t blen = strlen(base);
    if (blen > 4 && strcmp(base + blen - 4, ".tiq") == 0) blen -= 4;
    if (blen == 0) { base = "lib"; blen = 3; }
    FILE *out = ctx->out;
    fputs("/* Generated by tiq emit-header from ", out);
    fwrite(base, 1, blen, out);
    fputs(". Do not edit. */\n", out);
    fputs("/* Returned str values are Tiq-owned: callers must not free them (leak, never dangle). */\n", out);
    fputs("#ifndef TIQ_", out);
    fputs_header_guard(out, base, blen);
    fputs("_H\n#define TIQ_", out);
    fputs_header_guard(out, base, blen);
    fputs("_H\n\n#include <stdint.h>\n\n#ifdef __cplusplus\nextern \"C\" {\n#endif\n\n", out);
    emit_struct_defs(ctx, stmts, count);
    for (int i = 0; i < count; i++) {
        if (stmts[i] && function_exportable(stmts[i])) {
            emit_semantic_type(stmts[i]->semantic_type, out);
            fprintf(out, " %.*s(", (int)stmts[i]->as.function.name.length,
            ...
            if (stmts[i]->as.function.param_count == 0) {
                ...
                for (int j = 0; j < stmts[i]->as.function.param_count; j++) {
                    ...
                    emit_semantic_type(pt, out);
                    fputs(" ", out);
                    fprintf(out, "%.*s", (int)stmts[i]->as.function.params[j].length,
...
void compile_modules_to_c(SemanticModule *mods, int mod_count, const char *root_path,
    ...
    // M16.3: library modes require definitions-only modules. The first
    // top-level executable statement fails closed (E31), byte-identical
    // in both compilers.
    if (mode != TIQ_EMIT_PROGRAM) {
        for (int i = 0; i < count; i++) {
            AstNode *s = stmts[i];
            if (s && s->kind != AST_FUNCTION && s->kind != AST_STRUCT_DEF &&
                s->kind != AST_ENUM_DEF && s->kind != AST_EXTERN) {
                diag_error(diag, root_path, s->token.line, ERR_LIBRARY,
                           "library requires definitions only; top-level statement is not allowed");
                break;
            }
        }
        if (diag->has_error) { free(stmts); free(stmt_std); type_pool_free(&pool); return; }
    ...
    // M16.3: header mode emits only the C header and stops.
    if (mode == TIQ_EMIT_HEADER) {
        emit_c_header(ctx, stmts, count, root_path);
        free(stmts); free(stmt_std); type_pool_free(&pool);
        return;
    }
    ...
    // M16.3: lib mode omits the entry point; the emitted definitions
    // link into a host program that owns main().
    if (mode != TIQ_EMIT_LIB) {
...
void compile_to_c(const char *source_path, const char *source, FILE *out, DiagContext *diag) {
    ...
    if (!diag->has_error) {
        ...
        compile_modules_to_c(&mod, 1, source_path, out, diag, TIQ_EMIT_PROGRAM);
    }
    parser_free(&parser);
}
