...
// Virtual address of a defined symbol, based on the section it lives in.
// The per-object section base offsets are computed during sizing; text lives
// in the R+E segment, rodata in the same segment, data/bss in the RW segment.
...
int link_elf_exec(const ElfObject *objs, size_t nobj, const char *entry,
    ...
    *out_len = 0;
    ...
    // Align into the PLT page (plt_file_off = align_up(text_file_off + total_text, 16)).
    // Without this, a non-16-aligned total_text shifts the PLT (and plt_vm)
    // apart, making redirected calls land in PLT NOP padding.
    buf_pad(&b, 16);
            ...
            if (is_x86_64) {
                ...
                switch (rel->type) {
                    ...
                    case ASM_RELOC_X86_64_64: {
                        ...
                        if (sym_defined) {
                            const ElfSymbol *def = &objs[sym_oi].symbols[sym_si];
                            uint64_t sym_vm = symbol_vm(text_off, rodata_off, data_off, bss_off, sym_oi, def, text_vm_off, rodata_vm, data_vm, bss_seg_off);
                            val += sym_vm;
                        }
                        wr_u64(dloc, val);
                        break;
                ...
                switch (rel->type) {
                    case ASM_RELOC_BRANCH26: {
                        if (sym_defined) {
                            const ElfSymbol *def = &objs[sym_oi].symbols[sym_si];
                            uint64_t target = symbol_vm(text_off, rodata_off, data_off, bss_off, sym_oi, def, text_vm_off, rodata_vm, data_vm, bss_seg_off);
